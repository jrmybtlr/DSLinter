...
The dashboard + CLI wrapper is [`dashboard`](dashboard/). Published as **`dslinter`** on npm.
...
```bash
pnpm run build:napi
node dashboard/bin/dslinter.mjs --version
```

[`dashboard/index.cjs`](dashboard/index.cjs) is generated by NAPI-RS and auto-patched after each build (`scripts/patch-napi-loader.mjs`) so native binding version checks read `package.json` at runtime. Do not edit version strings in that file by hand.
