...
import { REPORT_URL_PATH } from "../shared/paths.ts";
import { resolveReportFilePath } from "../shared/reportPath.ts";
import { collectScanModuleRelPaths, embedGlobKeyFromRelPath } from "./collectScanModules.ts";
import {
  buildEmbedIndexCss,
...
function generatePlaygroundModulesSource(scanRoot: string, relPaths: string[]): string {
  const root = resolve(scanRoot);
  const lines: string[] = ["// Generated by dslinter/vite — do not edit"];
 ...
 * ```
 */
export default function dslinter(options: DslinterVitePluginOptions = {}): Plugin {
  const scanRoot = resolve(options.scanRoot ?? process.env.DSLINTER_SCAN_ROOT ?? process.cwd());
  const consumerViteRoot = resolve(
    options.consumerViteRoot ?? process.env.DSLINTER_CONSUMER_VITE_ROOT ?? "",
  );
  const servePort = options.servePort ?? resolveServePort();
  const consumerRoot = consumerViteRoot && existsSync(consumerViteRoot) ? consumerViteRoot : null;
  ...
  return {
    ...
    config(config, { mode }): UserConfig {
      ...
      const fsAllow = Array.isArray(existingAllow) ? [...existingAllow, ...allowRoots] : allowRoots;
    ...
    async configResolved(config) {
      ...
      try {
        ...
        consumerAliases = loadConsumerAliases(consumerRoot, loaded?.config?.resolve?.alias);
      } catch {
    ...
    resolveId(id, importer) {
      ...
      if (!consumerRoot) {
        ...
        if (isWayfinderRoutesImport(id) || isWayfinderActionsImport(id)) {
          const onDisk = resolveExistingModule(id, consumerAliases);
          if (onDisk) return onDisk;
          const shim = resolveWayfinderShim(id, wayfinderRoutesShimPath, wayfinderActionsShimPath);
          if (shim) return shim;
        }
      }
