  ...
  if (process.env.NAPI_RS_NATIVE_LIBRARY_PATH) {
    ...
    if (process.arch === 'arm64') {
      ...
      try {
        ...
        if (bindingPackageVersion !== __napiExpectedVersion && process.env.NAPI_RS_ENFORCE_VERSION_CHECK && process.env.NAPI_RS_ENFORCE_VERSION_CHECK !== '0') {
          throw new Error(`Native binding package version mismatch, expected ${__napiExpectedVersion} but got ${bindingPackageVersion}. You can reinstall dependencies to fix this issue.`)
        }
        return binding
      } catch (e) {
        loadErrors.push(e)
...
if (!nativeBinding || forceWasi) {
  ...
  if (!wasiBindingLoaded && (!__napiWasiFlavorRequested || __napiWasiFlavor === "wasm32-wasi")) {
    ...
    try {
      candidateError = __napiWasiResolveCandidate('./dslinter.wasi.cjs', false, ["./dslinter.wasm32-wasi.debug.wasm","./dslinter.wasm32-wasi.wasm"])
  ...
  if (!wasiBindingLoaded && (!__napiWasiFlavorRequested || __napiWasiFlavor === "wasm32-wasi")) {
    ...
    let candidateFailed = false
    try {
      candidateError = __napiWasiResolveCandidate('@dslinter/binding-wasm32-wasi', true, undefined)
      candidateFailed = candidateError !== null
      if (!candidateFailed) {
        if (process.env.NAPI_RS_ENFORCE_VERSION_CHECK && process.env.NAPI_RS_ENFORCE_VERSION_CHECK !== '0') {
          const bindingPackageVersion = require('@dslinter/binding-wasm32-wasi/package.json').version
          if (bindingPackageVersion !== __napiExpectedVersion) {
