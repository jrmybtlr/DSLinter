  ...
  if (components.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-border bg-muted/30 px-4 py-8 text-center text-sm text-muted-foreground">
        No unused components — every scanned definition has at least one JSX reference in the
        workspace.
      </p>
    );
  }
          ...
          <TableRow key={name}>
            ...
            <TableCell className="min-w-0 font-mono text-xs text-muted-foreground">
              {definitionPaths.length === 1 ? (
                <TruncatedPath path={shortPath(root, definitionPaths[0]!)} className="text-xs" />
              ) : (
                <span title={definitionPaths.map((p) => shortPath(root, p)).join("\n")}>
                  {definitionPaths.length} {pluralize("file", definitionPaths.length)}
                </span>
              )}
            </TableCell>
          </TableRow>
