...
const SAMPLE = "resources/js/components/manage-two-factor.tsx";

describe("resolveReportAbsolutePath", () => {
  it("returns absolute paths unchanged when under root", () => {
    expect(resolveReportAbsolutePath("/repo", "/repo/src/components/Button.tsx")).toBe(
      "/repo/src/components/Button.tsx",
    );
  });

  it("joins root-relative paths", () => {
    expect(resolveReportAbsolutePath("/repo", "src/pages/Home.tsx")).toBe(
      "/repo/src/pages/Home.tsx",
    );
  });
});
...
describe("truncatePathMiddle", () => {
  ...
  it("drops middle directories while keeping filename", () => {
    expect(truncatePathMiddle(SAMPLE, 41)).toBe("resources/js/.../manage-two-factor.tsx");
  });

  it("progressively collapses more leading segments", () => {
    expect(truncatePathMiddle(SAMPLE, 37)).toBe("resources/.../manage-two-factor.tsx");
    expect(truncatePathMiddle(SAMPLE, 27)).toBe(".../manage-two-factor.tsx");
  });

  it("normalizes backslashes", () => {
    expect(truncatePathMiddle("resources\\js\\components\\manage-two-factor.tsx", 100)).toBe(
      SAMPLE,
    );
  });
