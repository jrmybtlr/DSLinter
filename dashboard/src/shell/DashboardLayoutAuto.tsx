...
function StaticPreviewUnavailableBanner() {
  return (
    <div
      ...
      role="status"
    >
      <strong className="font-medium">Governance only</strong> — live component previews are
      unavailable in the prebuilt dashboard. Run{" "}
      <code className="rounded bg-muted px-1 py-0.5 font-mono text-xs">npx dslinter</code> from your
      project root and open the <strong>Dashboard</strong> URL from the terminal banner (port 5175),
      not the scanner-only port. Upgrade to the latest{" "}
      <code className="rounded bg-muted px-1 py-0.5 font-mono text-xs">dslinter</code> if previews
      still fail.
    </div>
  );
}
...
export default function DashboardLayoutAuto(props: DashboardLayoutProps) {
  ...
  const showStaticPreviewWarning =
    isStaticBundledPreviewUnavailable(autoPlaygroundBuild.skipped) ||
    (autoPlaygroundBuild.entries.length === 0 &&
      autoPlaygroundBuild.skipped.length > 0 &&
      autoPlaygroundBuild.skipped.every((s) => s.reason === "module_not_found"));
