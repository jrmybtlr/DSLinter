...
const ICON_VARIANTS = new Set(["icon", "icon-outline"]);
const NON_SOLID_VARIANTS = new Set([
    "ghost",
    "outline",
    "danger-outline",
    "icon",
...
function pickSizeClasses(variant, size) {
    if (ICON_VARIANTS.has(variant)) {
        const key = size in styles.iconSizes ? size : "default";
        return styles.iconSizes[key];
    }
    const key = size in styles.sizes ? size : "default";
 ...
 */
export function Button({ variant = "white", size = "default", loading = false, disabled, className, onClick, children, as, href, ...rest }) {
    const v = variant in styles.variants ? variant : "white";
    const isDisabled = disabled ?? loading;
    const isIconVariant = ICON_VARIANTS.has(v);
    const classes = cn(styles.base, !NON_SOLID_VARIANTS.has(v) && styles.solid, styles.variants[v], pickSizeClasses(v, size), className);
    const renderAsAnchor = as === "a" || (as !== "button" && href !== undefined);
    if (renderAsAnchor) {
        const anchorProps = rest;
        return (<a {...anchorProps} href={href} onClick={isDisabled ? (e) => e.preventDefault() : onClick} data-disabled={isDisabled || undefined} aria-disabled={isDisabled || undefined} className={cn("relative inline-flex shrink-0 cursor-pointer items-center justify-center gap-x-1.5", !isIconVariant &&
                "[&>svg:nth-child(1)]:-ml-1.5 [&>svg:nth-child(2)]:-mr-1.5", classes)}>
        <TouchTarget>{children}</TouchTarget>
      </a>);
    }
    const buttonProps = rest;
    return (<button type={buttonProps.type ?? "button"} {...buttonProps} onClick={isDisabled ? (e) => e.preventDefault() : onClick} disabled={isDisabled || undefined} data-disabled={isDisabled || undefined} aria-busy={loading || undefined} className={cn("relative shrink-0 cursor-default", classes)}>
      <TouchTarget>
        <span className={cn("pointer-events-none absolute inset-0 flex items-center justify-center opacity-0", loading && "opacity-100")} aria-hidden>
          <LoadingSpinner />
        </span>
        <span className={cn("inline-flex min-w-0 items-center justify-center gap-x-1.5", !isIconVariant &&
            "[&>svg:nth-child(1)]:-ml-1.5 [&>svg:nth-child(2)]:-mr-1.5", loading && "opacity-0")}>
          {children}
        </span>
      </TouchTarget>
    </button>);
...
function LoadingSpinner() {
    return (<span aria-hidden className="inline-block size-4 shrink-0 animate-spin rounded-full border-2 border-current border-t-transparent"/>);
}
/** Expand the hit area to at least 44x44px on touch devices. */
export function TouchTarget({ children }) {
    return (<>
      {children}
      <span aria-hidden className="absolute top-1/2 left-1/2 size-[max(100%,2.75rem)] -translate-1/2 pointer-fine:hidden"/>
    </>);
}
